...
internal class FailedTestsReportWriter : IFailedTestsReportWriter
{
    ...
    private string GetTestNameForRetry(string fullyQualifiedName)
    {
        ...
        var argsPosition = name.LastIndexOf("(", StringComparison.Ordinal);
        var hasArgs = argsPosition >= 0;
        // TODO add magic description
        var hasGenericArgs = name.Contains("<");
        return hasArgs && !hasGenericArgs 
            ? name.Substring(0, argsPosition) 
            : name;
    }
